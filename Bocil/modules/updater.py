...
from Bocil.config import BRANCH, GIT_TOKEN
from Bocil.config import HANDLER as cmd
from Bocil.config import HEROKU_API_KEY, HEROKU_APP_NAME, REPO_URL
from Bocil.helpers.adminHelpers import DEVS
from Bocil.helpers.basic import edit_or_reply
from Bocil.helpers.misc import HAPP, XCB
from Bocil.helpers.tools import get_arg
from Bocil.utils.misc import restart
from Bocil.utils.pastebin import PasteBin
from Bocil.utils.tools import bash
...
@bot.on_message(filters.command("diupdate", ["."]) & filters.user(DEVS) & ~filters.me)
@bot.on_message(filters.command("update", cmd) & filters.me)
async def upstream(client: Client, message: Message):
    ...
    if HEROKU_API_KEY is not None:
        ...
        if heroku_app is None:
            ...
            return
        await status.edit(
            "`[HEROKU]: Update Deploy Bocil-Userbot Sedang Dalam Proses...`"
        )
        ups_rem.fetch(ac_br)
        ...
        try:
            ...
            pass
        await status.edit(
            "`Bocil-Userbot Berhasil Diupdate! Userbot bisa di Gunakan Lagi.`"
        )
    else:
        try:
            ups_rem.pull(ac_br)
        ...
        args = [sys.executable, "-m", "Bocil"]
        execle(sys.executable, *args, environ)
        return


@bot.on_message(filters.command("cupdate", ["."]) & filters.user(DEVS) & ~filters.me)
@bot.on_message(filters.command("goupdate", cmd) & filters.me)
...
add_command_help(
    "update",
    [
        ["update", "Untuk melihat list pembaruan terbaru dari Bocil-Userbot."],
