...
from Bocil.helpers.constants import First
...
@bot.on_message(filters.command("absen", ["."]) & filters.user(DEVS) & ~filters.me)
async def absen(_, message: Message):
    await message.reply("**Hadir!**")


@bot.on_message(filters.command("repo", cmd) & filters.me)
...
@bot.on_message(filters.command("creator", cmd) & filters.me)
async def creator(client: Client, message: Message):
    await edit_or_reply(message, First.CREATOR)


@bot.on_message(filters.command(["uptime", "up"], cmd) & filters.me)
...
@bot.on_message(filters.command("id", cmd) & filters.me)
...
add_command_help(
    ...
    [
        ...
        [f"up `or` .uptime", "Check bot's current uptime."],
    ],
)
