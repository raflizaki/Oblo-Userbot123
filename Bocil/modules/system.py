...
from Bocil.helpers.misc import HAPP

from .help import add_command_help


@bot.on_message(filters.command("restart", cmd) & filters.me)
...
@bot.on_message(filters.command("shutdown", cmd) & filters.me)
async def shutdown_bot(client: Client, message: Message):
    if BOTLOG_CHATID:
        await client.send_message(
            BOTLOG_CHATID,
            "**#SHUTDOWN** \n"
            "**Bocil-Userbot** telah di matikan!\nJika ingin menghidupkan kembali silahkan buka heroku",
        )
    await edit_or_reply(message, "**Bocil-Userbot Berhasil di matikan!**")
    if HAPP is not None:
...
@bot.on_message(filters.command("logs", cmd) & filters.me)
async def logs_ubot(client: Client, message: Message):
    ...
    await client.send_document(
        message.chat.id,
        "Logs-Heroku.txt",
        thumb="Bocil/resources/logo.jpg",
