...
from Bocil import LOGGER
from Bocil.config import BRANCH, GIT_TOKEN, HEROKU_API_KEY, HEROKU_APP_NAME, REPO_URL
...
def git():
    ...
    try:
        repo = Repo()
        LOGGER("Bocil").info(f"Git Client Found")
    except GitCommandError:
        LOGGER("Bocil").info(f"Invalid Git Command")
        ...
        LOGGER("Bocil").info("Fetched Latest Updates")
    ...
    if is_heroku:
        if HEROKU_API_KEY and HEROKU_APP_NAME:
            try:
                ...
                LOGGER("Bocil").info(f"Heroku App Configured")
...
async def create_botlog(client):
    if HAPP is None:
        return
    LOGGER("Bocil").info("TUNGGU SEBENTAR. SEDANG MEMBUAT GROUP LOG USERBOT UNTUK ANDA")
    desc = "Group Log untuk Bocil-Userbot.\n\nHARAP JANGAN KELUAR DARI GROUP INI.\n\n✨ Powered By ~ @pantekyks ✨"
    try:
